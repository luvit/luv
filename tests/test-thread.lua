return require('lib/tap')(function (test)
  ...
  test("join after the loop has joined the thread", function(_, p, _, uv)
    local thread = uv.new_thread(function() end)
    uv.run()
    assert(thread:join() == true)
    assert(thread:join() == true)
  end)

  test("join and detach after detach", function(_, p, _, uv)
    local thread = uv.new_thread(function() end)
    assert(thread:detach() == true)
    local ret, err = thread:join()
    assert(ret == nil and err:match("^EINVAL"), err)
    ret, err = thread:detach()
    assert(ret == nil and err:match("^EINVAL"), err)
  end, "1.50.0")

  test("detach after the loop has joined the thread", function(_, p, _, uv)
    local thread = uv.new_thread(function() end)
    uv.run()
    assert(thread:detach() == true)
  end, "1.50.0")
